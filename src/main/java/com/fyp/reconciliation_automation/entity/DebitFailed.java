...
@Entity
@Table(
        name = "nibss_debit_failed",
        schema = "recon",
        uniqueConstraints = {
                @UniqueConstraint(columnNames = "session_id", name = "debit_failed_session_id"),
                @UniqueConstraint(columnNames = "transaction_id", name = "debit_failed_transaction_id")
        }
)
public class DebitFailed extends AbstractPersistableEntity {


    @Id
    @GeneratedValue(strategy = GenerationType.SEQUENCE, generator = "debit_failed_seq")
    @SequenceGenerator(name = "debit_failed_seq", sequenceName = "debit_failed_id_seq", allocationSize = 2000)
    private Long id;
