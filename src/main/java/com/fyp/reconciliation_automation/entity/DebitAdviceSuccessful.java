...
@Entity
@Table(
        name = "nibss_debit_advice_successful",
        schema = "recon",
        uniqueConstraints = {
                @UniqueConstraint(columnNames = "session_id", name = "debit_advice_successful_session_id"),
                @UniqueConstraint(columnNames = "transaction_id", name = "debit_advice_successful_transaction_id")
        }
)
public class DebitAdviceSuccessful extends AbstractPersistableEntity {


    @Id
    @GeneratedValue(strategy = GenerationType.SEQUENCE, generator = "debit_advice_successful_seq")
    @SequenceGenerator(name = "debit_advice_successful_seq", sequenceName = "debit_advice_successful_id_seq", allocationSize = 2000)
    private Long id;
